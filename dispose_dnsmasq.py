...
class DnsmasqRuleParser:
    ...
    def filter_valid_rules(self):
        ...
        # 生成 all-lite-dnsmasq.txt 文件，仅包含国内 DNS 解析成功的域名
        lite_rules = []
        ...
        # 保存 all-lite-dnsmasq.txt 文件
        lite_output_file = "all-lite-dnsmasq.txt"
            ...
            for comment in self.header_comments:
                if comment.startswith("# Title:"):
                    if "# Title:" not in seen_titles:  # 确保只写入一次 Title
                        f.write("# Title: cloudyun-check-lite-dnsmasq\n")
    ...
    def save_rules(self, rules):
        ...
        with open(self.output_file, "w", encoding="utf-8") as f:
            ...
            for comment in self.header_comments:
                if comment.startswith("# Title:"):
                    if "# Title:" not in seen_titles:  # 确保只写入一次 Title
                        f.write("# Title: cloudyun-AD-rules-check-dnsmasq\n")
                        seen_titles.add("# Title:")
                elif comment.startswith("# Total lines:"):
                    if "# Total lines:" not in seen_titles:  # 确保只写入一次 Total lines
...
if __name__ == "__main__":
    input_file = "beforeall_dnsmasq.txt"  # 输入文件
    output_file = "all-dnsmasq.txt"  # 输出文件

    # 解析规则并过滤
    parser = DnsmasqRuleParser(input_file, output_file)
    parser.parse_rules()
